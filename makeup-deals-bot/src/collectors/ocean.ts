/**
 * Coleta produtos da Ocean/Océane via Rakuten Product Search API.
 */
...
const SEARCH_KEYWORDS = [
  "batom", "base", "paleta", "blush", "iluminador",
  "máscara de cílios", "delineador", "corretivo",
...
export async function collectFromOcean(
  onBatch?: (products: RawProduct[]) => Promise<void>
): Promise<RawProduct[]> {
  const mid = getAdvertiserId("ocean");
  if (!mid) {
    logger.warn("[Ocean] MID não configurado.");
    return [];
  }
  ...
  for (const keyword of SEARCH_KEYWORDS) {
    ...
    logger.info(`[Ocean] "${keyword}": ${items.length} resultados, ${onSale.length} em promo`);
    all.push(...onSale);

    if (onBatch && onSale.length > 0) await onBatch(onSale);
  }

  logger.info(`[Ocean] Total: ${all.length} produtos em promo.`);
  return all;
}
