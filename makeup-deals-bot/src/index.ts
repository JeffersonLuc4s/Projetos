...
async function bootstrap() {
  logger.info("🚀 Iniciando Makeup Deals Bot...");
  ...
  // 3. Scheduler (desabilitado via SCHEDULER_ENABLED=false)
  if (process.env.SCHEDULER_ENABLED !== "false") {
    startScheduler();
  } else {
    logger.info("[Boot] Scheduler desabilitado (SCHEDULER_ENABLED=false).");
  }
